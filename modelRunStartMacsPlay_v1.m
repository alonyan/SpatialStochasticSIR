...
R = MulticolorSPIMResults('/mnt/RazorScope/RazorScopeSets/Zach/CorneaWound/CorneaWound24h_2018Oct26/acq_4')

%% Make spatial arangement of cells
A = R.SPIMTimepoints{2}.Centroids(1:4:end,:);%(1000:12000,:);
nCells = size(A,1);
...
h(1) = scatter(A(logical(x(1:nSpecies:end)),1),A(logical(x(1:nSpecies:end)),3),30,1./Lifetimes(logical(x(1:nSpecies:end))),'filled','MarkerFaceAlpha', 0.2);
set(ax1,'xcolor','none','ycolor','none','CLim',[0,1]);
colormap('parula')
hold on;
h(2) = scatter(A(logical(x(2:nSpecies:end)),1),A(logical(x(2:nSpecies:end)),3),30,'r','filled','MarkerFaceAlpha', 0.8);
h(3) = scatter(A(logical(x(3:nSpecies:end)),1),A(logical(x(3:nSpecies:end)),3),30,'k','filled','MarkerFaceAlpha', 0.4);
h(4) = scatter(A(logical(x(4:nSpecies:end)),1),A(logical(x(4:nSpecies:end)),3),30,'g','filled','MarkerFaceAlpha', 0.6);
h(5) = scatter(A(logical(x(5:nSpecies:end)),1),A(logical(x(5:nSpecies:end)),3),30,'y','filled','MarkerFaceAlpha', 0.6);
axis equal;
